...
destert_africa <- crop(desert_wgs84, africa_extent)
...
plot(precipitation_africa, main = "Precipitation Map of Africa", col = viridis(100), breaks = seq(0, 3000, by = 100))
...
for (country in cty_adm2) {
  ...
  country_results <- regions %>%
    as.data.frame() %>%
    rename(iso = GID_0) %>%
    select(iso, NAME_2, precipitation_adm2)
...
plot(temp_mean_africa, main = "temp_mean Map of Africa", col = rev(heat.colors(100)), breaks = seq(0, 50, by = 1))
...
for (country in cty_adm2) {
  ...
  country_results <- regions %>%
    as.data.frame() %>%
    rename(iso = GID_0) %>%
    select(iso, NAME_2, temp_mean_adm2)
...
for (country in cty_adm2) {
  ...
  country_results <- regions %>%
    as.data.frame() %>%
    rename(iso = GID_0) %>%
    select(iso, NAME_2, temp_max_adm2)
...
for (country in cty_adm2) {
  ...
  country_results <- regions %>%
    as.data.frame() %>%
    rename(iso = GID_0) %>%
    select(iso, NAME_2, temp_min_adm2)
...
for (country in cty_adm2) {
  ...
  country_results <- regions %>%
    as.data.frame() %>%
    rename(iso = GID_0) %>%
    select(iso, NAME_2, mountain_adm2)
