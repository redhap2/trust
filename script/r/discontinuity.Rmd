...
ggplot() +
  geom_sf(data = murdock_sf, aes (color="Murdock"), fill = "transparent", size = 1) + # Murdock map in black
  geom_sf(data = buffer, aes(color = "boundaries"), lwd = 1) + # 25km buffer around borders in red
  scale_color_manual(
...
ggsave("C:/Users/Redha CHABA/Documents/wp_git/rbci/plots/discontinuity/murdock_boundaries_map.jpg", width = 10, height = 6, dpi = 300)
```
...
ggsave("buffer_25.jpg", width = 10, height = 6, dpi = 300)

```

```{r}
data_trust_test <- readRDS("data/data_rds/data_27_02_25.rds")
data_trust_test%<>%
  ungroup()
