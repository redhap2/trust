...
countries <- unique(data_final$iso)
...
qog%<>%
  group_by(iso, round)%>%
  mutate(poly_mean = ifelse(is.na(round), NA_integer_, mean(vdem_polyarchy, na.rm = TRUE))) %>%
  mutate(med_corr_mean = ifelse(is.na(round), NA_integer_, mean(vdem_med_corrupt, na.rm = TRUE))) %>%
  ungroup()

qog %<>%
  filter(!is.na(round)) %>%
  distinct(iso, round, .keep_all = TRUE) %>%
  select(iso, round, poly_mean, med_corr_mean)
